	   ...
	   iTrack1 != thePATTrackHandle->end(); ++iTrack1 )
	       ...
	       iTrack2 != thePATTrackHandle->end(); ++iTrack2 ) 
		 {
		   ...
		   //if(iTrack2->pt()<0.5) continue;
	           if(fabs(iTrack2->eta())>2.5) continue;
		   ...
		   //********** doing according to Bfinder.cc
		   if(pipi4V.M() > PDG_PHI_MASS + 0.1) continue;
                   if(pipi4V.M() < PDG_PHI_MASS - 0.1) continue;

		   vector<RefCountedKinematicParticle> phiParticles;
		   phiParticles.push_back(pFactory.particle(pion1TT, pion_mass, chi,ndf, pion_sigma));
		   phiParticles.push_back(pFactory.particle(pion2TT, pion_mass, chi,ndf, pion_sigma));
		   ...
		   KinematicParticleVertexFitter   phifitter;
                   ...
                   double phi_Prob_tmp = TMath::Prob(PHIvtx->chiSquared(), (int)PHIvtx->degreesOfFreedom());
			...
			*/
		    ...
		    vector<RefCountedKinematicParticle> Bs_candidate_init;

                    Bs_candidate_init.push_back(pFactory.particle(muon1TT, muon_mass, chi,ndf, muon_sigma));
                    Bs_candidate_init.push_back(pFactory.particle(muon2TT, muon_mass, chi,ndf, muon_sigma));
                    Bs_candidate_init.push_back(pFactory.particle(pion1TT, pion_mass, chi,ndf, pion_sigma));
                    Bs_candidate_init.push_back(pFactory.particle(pion2TT, pion_mass, chi,ndf, pion_sigma));
                    RefCountedKinematicTree xbVFT, BsFitTree;

                    vector<RefCountedKinematicParticle> Bs_candidate = Bs_candidate_init;
		    ...
		    ///////////////////// BS VERTEX FIT /////////////////////////////////////////
		    // Jpsi mass constraints

                    Bs_candidate = Bs_candidate_init;
                    MultiTrackKinematicConstraint *ConstraintJpsiMass = new TwoTrackMassKinematicConstraint(psi_mass);
...
*/
		   ...
		   BsFitTree->movePointerToTheFirstChild();
                   ...
                   BsFitTree->movePointerToTheNextChild();
                   RefCountedKinematicParticle T1CandMC     = BsFitTree->currentParticle();
                   BsFitTree->movePointerToTheNextChild();
                   RefCountedKinematicParticle T2CandMC     = BsFitTree->currentParticle();
		   ...
		   for (unsigned int i = 0, n = thePATTrackHandleT.size(); i<n ; ++i )
		     {
		       ...
		       //if(iTrack3->pt()<0.2) continue;
	               if(fabs(iTrack3->eta())>2.5) continue;
		       ...
		       Bs4V.SetXYZM(BsCandCjp->currentState().globalMomentum().x(),BsCandCjp->currentState().globalMomentum().y(),BsCandCjp->currentState().globalMomentum().z(), BsCandCjp->currentState().mass()); 
			...
			*/
		       ...
		       vFitMCParticlesBspi.push_back(vFactory.particle(BsCandCjp->currentState(),Bs_fit_chi2,Bs_dof,BsCandCjp));
			...
			*/
			 ...
			 continue;
		       ...
		       //ye wale se analysis kar raha tha->if(bspiCandMC->currentState().mass()<6.26|| bspiCandMC->currentState().mass()>6.28) continue; // Good Bc and pion is also matching- This mass window applied for MC and data- using this I also working with BDT.
			...
			============= */
			 ...
			 event = event1;
		       ...
		       deltaMass->push_back( bspiCandMC->currentState().mass() - BsCandCjp->currentState().mass() ); //+ bs_mass );
		       ...
		       B_mass->push_back(BsCandCjp->currentState().mass());
		       B_px->push_back(BsCandCjp->currentState().globalMomentum().x());
		       B_py->push_back(BsCandCjp->currentState().globalMomentum().y());
		       B_pz->push_back(BsCandCjp->currentState().globalMomentum().z());
		       B_pt->push_back(BsCandCjp->currentState().globalMomentum().perp());
		       B_eta->push_back(BsCandCjp->currentState().globalMomentum().eta());
		       B_phi->push_back(BsCandCjp->currentState().globalMomentum().phi());
		       ...
		       B_chi2->push_back(BsCandCjp->chiSquared());
		       
		       //double B_Prob_tmp       = TMath::Prob(bDecayVertexMC->chiSquared(),(int)bDecayVertexMC->degreesOfFreedom());
		       //double J_Prob_tmp   = TMath::Prob(psi_vFit_vertex_noMC->chiSquared(),(int)psi_vFit_vertex_noMC->degreesOfFreedom());
		       B_Prob    ->push_back(Bs_Prob_tmp);
		       B_J_Prob  ->push_back(J_Prob_tmp);
		       
		       B_DecayVtxX ->push_back(BsDecayVertexCjp->position().x());    
		       B_DecayVtxY ->push_back(BsDecayVertexCjp->position().y());
		       B_DecayVtxZ ->push_back(BsDecayVertexCjp->position().z());
		       B_DecayVtxXE ->push_back(BsDecayVertexCjp->error().cxx());   
		       B_DecayVtxYE ->push_back(BsDecayVertexCjp->error().cyy());   
		       B_DecayVtxZE ->push_back(BsDecayVertexCjp->error().czz());
		       B_DecayVtxXYE ->push_back(BsDecayVertexCjp->error().cyx());
		       B_DecayVtxXZE ->push_back(BsDecayVertexCjp->error().czx());
		       B_DecayVtxYZE ->push_back(BsDecayVertexCjp->error().czy());
		       ...
		       saveIP(vertexFitTreeBspi,*pvHandle_.product(), pvIndex);
...
//	for (std::vector<int>::size_type j = 0; j < B_mass->size(); j++) { 
		...
		//double TruthMatchPionMaxR_ = 0.004;
		double TruthMatchPionMaxR_ = 0.3;
